...
export const permissions: Record<Role, PermissionsByRole> = {
  ...
  COMPANY_GROUP_ADMIN(user, { can, cannot }) {
    can('manage', 'User', { companyGroupId: {
      $eq: user.companyGroupId,
    } })
    
    can('create', 'Company')
    can(['read', 'update', 'delete'], 'Company', { companyGroupId: { $eq: user.companyGroupId } })
    can('manage', 'Unit', { companyGroupId: { $eq: user.companyGroupId } })
    can('manage', 'Base', { companyGroupId: { $eq: user.companyGroupId } })

    can('update', 'CompanyGroup', { id: { $eq: user.companyGroupId } })
    can('read', 'CompanyGroup', { id: { $eq: user.companyGroupId } })
  },
